...
const AuthFormCard: React.FC<AuthFormCardProps> = ({ onAuthSuccess }) => {
  ...
  /* 
   * Supabase Verification is usually Link-based. 
   * We will simply show a message to check email, or if OTP is enabled, verify it.
   * For now, we assume Link based, so we just prompt user.
   * If manual code entry is needed, we utilize supabase.auth.verifyOtp
   */
  const verifyEmail = async () => {
    // This function is largely redundant if we rely on Email Links.
    // But if we want to support OTP:
    setIsLoading(true);
    ...
    try {
      if (isSignUp) {
        ...
        const { data, error } = await supabase.auth.signUp({
          ...
          options: {
            data: {
              full_name: formData.email.split('@')[0], // Simplified name extraction
            },
        ...
        // If signup requires verification (default)
        if (data.user && !data.session) {
          setIsVerifying(true); // Show verification UI
          setIsLoading(false);
          return;
        }
      } else {
        ...
        if (data.session && data.user) {
          ...
          const storedUser = {
            ...user,
            name: user.user_metadata?.full_name || user.email?.split('@')[0], // Map metadata
          };
          ...
          // Load chats (using Backend API, passing bearer token if needed)
          try {
            const chatsResp = await fetch(
              `${process.env.NEXT_PUBLIC_BACKEND_URL}/chat/user/${user.id}`
              // TODO: Add Authorization header here once backend validates it
            );
            if (chatsResp.ok) {
          ...
          onAuthSuccess(storedUser, false);
        }
      }
    } catch (error: any) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/auth/callback` // Ensure you have this route or handled by default
        }
      });
      if (error) throw error;
    } catch (error: any) {
      ...
      setFormErrors((prev) => ({
        ...prev,
        submit: "Failed to sign in with Google: " + error.message,
      }));
      setIsLoading(false);
    }
  };
  ...
  const handleGitHubSignIn = async () => {
    ...
    try {
      ...
      setFormErrors((prev) => ({
        ...prev,
        submit: "Failed to sign in with GitHub: " + error.message,
      }));
      setIsLoading(false);
    }
  };
